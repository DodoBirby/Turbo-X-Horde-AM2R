...
if (active == 1)
{
    ...
    if (state == 3)
    {
        ...
        if ((canspmove || canspmove2) && statetime > 60 && spdelay == 0 && dodging == 0)
        {
            ...
            if (nextsp == 1)
            {
                if (distance_to_object(oCharacter) < 64 && room != rm_a4h09 && room != rm_a4h11)
                {
                    state = 7
                    spdelay = 180
                    statetime = 0
...
if (state == 20)
{
    ...
    if floor((distance_to_point(304, 432) < 1 || statetime == 300))
    {
        ...
        xVel = 0
        yVel = 0
        mtr = instance_create(304, 432, oMZeta_Cocoon)
        mtr.facing = facing
        mtr.state = 1
        mtr.statetime = 0
        mtr.myid = 52
...
if (myid == 52 && global.event[331] >= 3 && (!dead) && state < 20)
{
    if instance_exists(grabber)
    {
        with (grabber)
