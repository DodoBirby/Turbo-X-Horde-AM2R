...
if (state == 0)
{
    ...
    if (myid == 52 && instance_number(oMonster) == 1)
    {
        if (point_distance(x, y, oCharacter.x, (oCharacter.y - 16)) < 90)
        {
            state = 1
...
{
    instance_destroy()
    instance_create(x, (y + 16), oMEgg1)
    amet = instance_create(x, (y + 16), oMAlphaTriggerFirstCocoon)
    amet.facing = facing
    amet.myid = 52
    amet.myid2 = 93
    amet.alarm[0] = 280
...
if (myid == 52 && global.event[331] >= 1 && (!dead) && state < 20)
{
    hp = 0
    state = 20
    statetime = -1
