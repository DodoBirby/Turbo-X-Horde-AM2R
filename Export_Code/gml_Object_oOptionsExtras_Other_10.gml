...
op[num_IceToggle] = instance_create(x, (y + (sep * tempcount++)), oOptionLR)
op[num_IceToggle].label = get_text("ExtrasMenu", "IceToggle")
op[num_IceToggle].optionid = num_IceToggle
op[num_Hyper] = instance_create(x, (y + (sep * tempcount++)), oOptionLR)
op[num_Hyper].label = get_text("ExtrasMenu", "HyperMode")
op[num_Hyper].optionid = num_Hyper
if (global.mod_espresso_unlocked == 0)
{
    op[num_IceToggle].enabled = 0
    canedit[num_IceToggle] = 0
    op[num_Hyper].enabled = 0
    canedit[num_Hyper] = 0
}
