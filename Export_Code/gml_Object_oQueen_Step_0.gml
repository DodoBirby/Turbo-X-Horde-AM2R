...
if (state == 4)
{
    if (statetime == 0)
    {
        ...
        head_attacks = (2 + xtreme)
        head_obj.speedmulti = 0.1
        head_obj.attackdelay = 120
        if (head_obj.myhealth <= ((head_obj.starthealth / 5) * 4))
        {
            head_attacks = (3 + xtreme)
            head_obj.speedmulti = 1
            head_obj.attackdelay = 95
        }
        if (head_obj.myhealth <= ((head_obj.starthealth / 5) * 3))
        {
            head_attacks = (4 + xtreme)
            head_obj.speedmulti = 1.5
            head_obj.attackdelay = 70
        }
        if (head_obj.myhealth <= ((head_obj.starthealth / 5) * 2))
        {
            head_attacks = (5 + xtreme)
            head_obj.speedmulti = 2
            head_obj.attackdelay = 50
        }
        if (head_obj.myhealth <= (head_obj.starthealth / 5))
        {
            head_attacks = (6 + xtreme)
            head_obj.speedmulti = 2.5
            head_obj.attackdelay = 50
        }
        if (head_obj.myhealth <= (head_obj.starthealth / 10))
        {
            head_attacks = (16 + xtreme)
            head_obj.speedmulti = 3
            head_obj.attackdelay = 25
        }
    }
    if (statetime == 10)
    {
