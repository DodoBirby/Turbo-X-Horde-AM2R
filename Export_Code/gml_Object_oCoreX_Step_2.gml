...
if (state == 6)
{
    ...
    if (image_xscale <= 0)
    {
        ...
        else if (room == rm_a5h01)
        {
            with (instance_create((oCharacter.x - 8), (oCharacter.y - 8), scr_itemsopen(oControl.mod_258)))
            {
                visible = false
                active = 1
            }
        }
        else if (room == rm_a8a11)
        {
            with (instance_create((oCharacter.x - 8), (oCharacter.y - 8), scr_itemsopen(oControl.mod_50)))
            {
                visible = false
                active = 1
            }
